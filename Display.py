...
class ValueDisplay: 
    ...
    def get_frame(self, values): 
        #print(str(values) + " " + str(self.names))
...
def combine_frames(top, bottom): 
    ...
    if topw != botw: 
        scale = (1.0 * topw)/botw
        bottom = cv2.resize(bottom, None, fx = scale, fy = scale, interpolation= cv2.INTER_NEAREST)
...
if __name__ == "__main__":
    ...
    while not done: 
        ...
        values = ds.solve_system(pilotInput) + [0.75, -0.75]
