...
current_avgs = [0 for i in range(num_mot + 2)]
...
motors = ["M1", "M2", "M3", "M4", "M5", "M6", "C1", "C2"]
...
large_cam = 0
combined = combine_frames(dis_1.get_frame(pilotInput), dis_2.get_frame(current_avgs))
'''
...
while done==False:
    ...
    claw1 = -1 * joystick.get_axis(2)
    if claw1 >= 10 * error_range: 
        claw1 = motor_speed
    elif claw1 <= -10 * error_range: 
        claw1 = -1 * motor_speed
    else: claw1 = 0 
    ...
    if (claw1 != current_avgs[num_mot] or claw2 != current_avgs[num_mot + 1]): 
        current_avgs[num_mot] = claw1
        current_avgs[num_mot + 1] = claw2
        toWrite = True 
    
    if current_size > num_val/2 and toWrite: 
        motorvalues = [int(round(1500 + 400 * number)) for number in current_avgs[:-2]] + [int(round(375 + 125 * number)) for number in current_avgs[num_mot:]]
